...
The Worker deployment prints a URL ending in `.workers.dev`. Paste it into **SpeakUp → Settings → Secure Coach API URL**. The app saves only that public Worker URL locally. It sends completed audio and, only when the local face-landmark check is high confidence, a single compressed camera frame to `/api/analyze`.
